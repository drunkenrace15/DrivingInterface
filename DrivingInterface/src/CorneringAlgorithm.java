...
//		System.out.println("angle_base"+angle_base + ", data.track_dist_straight"+data.track_dist_straight);
		//코너링 전에 우회전일경우 우측으로 붙고, 좌회전일 경우 좌측으로 붙고
		
		
		if ( data.track_dist_straight >15 && data.track_dist_straight < 80  ) {		
			System.out.println("직진 중 : "+ angle_now+ "///// angle_base"+ angle_base+ "//////" +"data.speed" + data.getKMhSpeed());
...
//			//System.out.println("data.dest_Speed" + data.dest_Speed);
			...
			if(data.track_curve_type == DrivingInterface.curve_type_right){
				System.out.println("qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq"+ angle_base);
				if(angle_base > -9){
					data.dest_Speed = data.dest_Speed -4;
					m_angle = 0.9;
				}else if(angle_base > -7){
					data.dest_Speed = data.dest_Speed -3;
					m_angle = 0.7;
				}else if(angle_base > -5){
					data.dest_Speed = data.dest_Speed -2;
					m_angle = 0.5;
				}else if(angle_base >-3){
					data.dest_Speed = data.dest_Speed -1;
					m_angle = 0.3;
				}	
				System.out.println("우우회전중 anlge_now : "+ angle_now+ "///// angle_base"+ angle_base+ "//////" +"data.speed" + data.getKMhSpeed());
				data.dest_Middle = -m_angle * Math.abs(Math.sin(Math.PI/2 - (data.track_Front_angles[0] - data.track_Front_angles[1])) * data.toMiddle);
